...
/**
 * @version		$Id: view.php 23 2009-02-07 13:51:27Z chdemko $
...
abstract class LocaliseHelper
{
	...
	{
		if (JFactory::getConfig()->getValue('config.ftp_enable')) 
		{
			return true;
		}
		else
	...
	static protected function scanPackages() 
	{
		...
		$model = JModel::getInstance('Packages', 'LocaliseModel', array('ignore_request' => true));
	...
	static public function getScans($client = '', $type = '') 
	{
		...
		if (preg_match("/$filter_client/", 'administrator')) 
		{
			...
			if (preg_match("/$filter_type/", 'component')) 
			{
				...
				$scans[] = array('prefix' => '', 'suffix' => '.sys', 'type' => 'component', 'client' => 'administrator', 'path' => LOCALISEPATH_ADMINISTRATOR . '/components/', 'folder' => '');
			}
			if (preg_match("/$filter_type/", 'module')) 
			{
				...
				$scans[] = array('prefix' => '', 'suffix' => '.sys', 'type' => 'module', 'client' => 'administrator', 'path' => LOCALISEPATH_ADMINISTRATOR . '/modules/', 'folder' => '');
			}
			if (preg_match("/$filter_type/", 'template')) 
			{
				...
				$scans[] = array('prefix' => 'tpl_', 'suffix' => '.sys', 'type' => 'template', 'client' => 'administrator', 'path' => LOCALISEPATH_ADMINISTRATOR . '/templates/', 'folder' => '');
			}
		}
		if (preg_match("/$filter_client/", 'site')) 
		{
			...
			if (preg_match("/$filter_type/", 'component')) 
			{
				...
				$scans[] = array('prefix' => '', 'suffix' => '.sys', 'type' => 'component', 'client' => 'site', 'path' => LOCALISEPATH_SITE . '/components/', 'folder' => '');
			}
			if (preg_match("/$filter_type/", 'module')) 
			{
				// Scan site modules folders
				$scans[] = array('prefix' => '', 'suffix' => '', 'type' => 'module', 'client' => 'site', 'path' => LOCALISEPATH_SITE . '/modules/', 'folder' => '');
				$scans[] = array('prefix' => '', 'suffix' => '.sys', 'type' => 'module', 'client' => 'site', 'path' => LOCALISEPATH_SITE . '/modules/', 'folder' => '');
			}
			if (preg_match("/$filter_type/", 'template')) 
			{
				...
				$scans[] = array('prefix' => 'tpl_', 'suffix' => '.sys', 'type' => 'template', 'client' => 'site', 'path' => LOCALISEPATH_SITE . '/templates/', 'folder' => '');
			}
		}
		if (preg_match("/$filter_type/", 'plugin')) 
		{
			...
			foreach ($plugin_types as $plugin_type) 
			{
				...
				$scans[] = array('prefix' => 'plg_' . $plugin_type . '_', 'suffix' => '.sys', 'type' => 'plugin', 'client' => 'administrator', 'path' => JPATH_PLUGINS . "/$plugin_type/", 'folder' => '/administrator');
			}
		}
		return $scans;
	}
	public static function getFileId($path) 
	{
		...
		if (!isset($fileIds)) 
		{
			...
			$query->from($db->nameQuote('#__localise'));
			$db->setQuery($query);
...
/*		elseif ($filename == 'lib_joomla')
	...
	public static function parseSections($filename) 
	{
		...
		{
			//$this->setError($sections[$filename]['error']);
		}
